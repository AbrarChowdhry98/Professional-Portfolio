  ...
  const checks: { match: string[]; feature: Feature }[] = [
    {
      ...
      feature: {
        title: "Advanced Cart",
        description:
          "Complex logic for multi-item plans and dynamic pricing flows.",
    ...
    const defaults: Feature[] = [
      {
        title: "UI Systems",
        description:
          "Component-driven interfaces built for scale and maintainability.",
        icon: SpeedOutlinedIcon,
      },
      {
        title: "API Design",
        description:
          "Robust backend contracts with GraphQL and REST orchestration.",
...
function deriveHighlightLabel(keyword: string): string {
  const k = keyword.toLowerCase();

  if (["mongodb", "mysql", "postgresql", "redis"].some((d) => k.includes(d))) {
    return "Database";
  }
  if (
    [
  ...
  if (
    ["node.js", "express", "nestjs", "api", "rest", "graphql", "jwt"].some(
      (b) => k.includes(b),
    )
  ) {
    return "Back End";
  }
  if (["swift", "swiftui", "ios", "android"].some((m) => k.includes(m))) {
    return "Mobile";
  }
  if (["sitecore", "contentful", "cms"].some((c) => k.includes(c))) {
    return "Platform";
  }
  if (
    ["javascript", "typescript", "python", "java"].some((l) => k.includes(l))
  ) {
    return "Language";
  }
  if (["firebase", "aws", "gcp", "azure"].some((c) => k.includes(c))) {
    return "Cloud";
  }
...
function deriveScalabilityHighlights(keywords: string[] = []): {
  ...
  const primaryKeyword = keywords[0] ?? "Full Stack";
  const secondaryKeyword =
    keywords.find(
      (keyword, index) => index > 0 && keyword !== primaryKeyword,
    ) ??
    keywords[1] ??
    primaryKeyword;
  ...
  return {
    primary: {
      ...
      label: deriveHighlightLabel(primaryKeyword),
    },
    secondary: {
      value: secondaryKeyword,
      label: deriveHighlightLabel(secondaryKeyword),
    },
  };
}
...
function resolveFeatures(project: ProjectItem): Feature[] {
  if (project.features?.length) {
    return project.features.map((feature, index) => ({
      ...feature,
      icon: appFeatureIcons[index % appFeatureIcons.length],
    }));
  }

  return deriveFeatures(project.keywords);
}

function resolveScalabilityHighlights(project: ProjectItem): {
  primary: { value: string; label: string };
  secondary: { value: string; label: string };
} {
  ...
  return deriveScalabilityHighlights(project.keywords);
}
...
function resolveSpotlight(
  ...
  if (project.imageMode === "app") {
    return { heading: "Built for the Field", text: summary };
  }

  return {
    heading: "Architecting Scalability",
...
const ProjectPageView = ({
  ...
  const scalabilityHighlights = resolveScalabilityHighlights(project);
  const spotlight = resolveSpotlight(project, summary);
  const isAppProject = Boolean(project.gallery?.length);
  ...
  return (
    <main css={styles.page}>
      <section css={css([styles.section, styles.hero])}>
        ...
        {isAppProject ? (
          <div css={styles.heroAppIntro}>
            <h1 css={styles.title}>{main}</h1>
            <p css={styles.heroSummary}>{summary}</p>
            {project.link && (
              <div css={styles.heroAppActions}>
                <a
                  href={project.link}
                  target="_blank"
                  rel="noopener noreferrer"
                  css={styles.heroAppLink}
                >
                  View on App Store
            ...
            <div css={styles.imageCard}>
              {project.image ? (
                <>
                  ...
                  />
                  <div css={styles.heroScrim} aria-hidden="true" />
                </>
              ) : (
      ...
      {isAppProject && project.gallery && (
        <div css={styles.section}>
          <AppGallerySection
            gallery={project.gallery}
            subtext={project.gallerySubtext}
          />
        </div>
      )}
      ...
      <section css={css([styles.section, styles.featuresSection])}>
        <div css={styles.featuresWrapper}>
          ...
          />
          <h2 css={styles.featuresHeading}>
            {isAppProject ? "Key Capabilities" : "Engineered Features"}
          </h2>
      ...
      <section css={styles.section}>
        <div css={styles.scalability}>
          <div>
            <h2 css={styles.scalabilityHeading}>{spotlight.heading}</h2>
            <p css={styles.scalabilityText}>{spotlight.text}</p>
